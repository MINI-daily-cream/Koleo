...
namespace Koleo.Services
{
    public class ComplaintService : IComplaintService
    {
        ...
        {
            var sql = $"INSERT INTO Complaints (User_Id, Ticket_Id, Content) VALUES ('{userId}, '{ticketId}', '{content}')";
            var result = await _databaseService.ExecuteSQL(sql);
            return result.Item2;
        }

        public async Task<bool> EditComplaint(string complaintId, string ticketId, string content)
        {
            var sql = $"UPDATE Complaints SET Ticket_Id = {ticketId}, Content = '{content}' WHERE Id = '{complaintId}'";
            var result = await _databaseService.ExecuteSQL(sql);
            return result.Item2;
        }
        ...
        public async Task<(List<ComplaintWithAnswer>, bool)> ListComplaints(string userId)
        {
        ...
        {
            var sql = $"INSERT INTO AdminComplaints (Admin_Id, Complaint_Id, Response) VALUES ('{adminId}, '{complaintId}', '{response}')";
            var result = await _databaseService.ExecuteSQL(sql);
            return result.Item2;
        }

        public async Task<(List<ComplaintWithoutAnswer>, bool)> ListUnansweredComplaints()
        {
